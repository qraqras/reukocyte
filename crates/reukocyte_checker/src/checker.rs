...
impl<'rk> Checker<'rk> {
    ...
    // ========== Ancestor tracking ==========
    // NOTE: Rules are called BEFORE push_node, so during rule execution,
    // the "current" node in SemanticModel is actually the parent.
    // These methods provide the same API as before for backwards compatibility.

    /// Get the immediate parent node, if any.
    ///
    /// Note: This returns the "current" node in SemanticModel because rules
    /// are executed before push_node is called.
    #[inline]
    pub fn parent(&self) -> Option<&Node<'rk>> {
        // Rules run before push_node, so semantic's current_node IS the parent
        self.semantic.current_node()
    }

    /// Get the Nth ancestor (0 = parent, 1 = grandparent, etc.)
    ///
    /// Note: Adjusted for the fact that rules run before push_node.
    #[inline]
    pub fn ancestor(&self, n: usize) -> Option<&Node<'rk>> {
        // n=0 means parent, which is semantic's current_node
        // n=1 means grandparent, which is semantic's parent (ancestor(0))
        if n == 0 {
            self.semantic.current_node()
        } else {
            self.semantic.ancestor(n - 1)
        }
    }

    /// Check if any ancestor matches the given predicate.
    ///
    /// Note: This includes the "current" node in SemanticModel (which is the
    /// actual parent from the rule's perspective).
    ///
    /// # Example
    /// ```ignore
    ...
    where
        F: Fn(&Node<'rk>) -> bool,
    {
        // Include current_node (the actual parent) and its ancestors
        self.semantic.current_node().is_some_and(&predicate)
            || self.semantic.has_ancestor(&predicate)
    }
    ...
    where
        F: Fn(&Node<'rk>) -> bool,
    {
        // Check current_node first (the actual parent), then ancestors
        if let Some(current) = self.semantic.current_node() {
            if predicate(current) {
                return Some(current);
            }
        }
        self.semantic.find_ancestor(predicate)
    }
