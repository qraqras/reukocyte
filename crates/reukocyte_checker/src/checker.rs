...
/// A visitor that builds the node index and caches line/column in a single pass.
struct CombinedVisitor<'rk> {
    semantic: &'rk mut SemanticModel,
    line_index: &'rk LineIndex<'rk>,
    node_line_cache: &'rk std::cell::RefCell<rustc_hash::FxHashMap<usize, (usize, usize)>>,
}
impl<'rk> Visit<'rk> for CombinedVisitor<'rk> {
    fn visit_branch_node_enter(&mut self, node: Node<'rk>) {
        let offset = node.location().start_offset();
        let (line, col) = self.line_index.line_column(offset);
        self.node_line_cache.borrow_mut().insert(offset, (line, col));
...
pub struct Checker<'rk> {
    ...
    // Cache for matches_patterns hashed by (file_path + patterns content).
    // Stores the computed Option<bool> so repeated calls are cheap.
    matches_patterns_cache: std::cell::RefCell<rustc_hash::FxHashMap<u64, Option<bool>>>,
    // Cache of per-rule enabled values keyed by a `config path` string generated by build.rs
    // e.g. "layout.indentation_consistency". This avoids repeated include/exclude checks
    // for the same rule during a run.
    rule_enabled_cache: std::cell::RefCell<rustc_hash::FxHashMap<&'static str, bool>>,
    // Cached category flags set during initialization
    pub(crate) needs_ast: std::cell::Cell<bool>,
    pub(crate) needs_lines: std::cell::Cell<bool>,
    pub(crate) needs_file: std::cell::Cell<bool>,
    pub(crate) needs_tokens: std::cell::Cell<bool>,
    // Scratch buffers used to avoid allocations in hot paths like alignment.
    scratch_offsets: Vec<(usize, usize)>,
    scratch_resolved_line_info: Vec<(usize, usize, usize, usize, usize)>,
    scratch_reports: Vec<(usize, usize, isize)>,
    // Cache for node line/column to avoid repeated calculations
    node_line_cache: std::cell::RefCell<rustc_hash::FxHashMap<usize, (usize, usize)>>, // offset -> (line, column)
}
impl<'rk> Checker<'rk> {
    ...
    pub fn new(source: &'rk [u8], config: &'rk Config) -> Self {
        let checker = Self {
            ...
            matches_patterns_cache: std::cell::RefCell::new(rustc_hash::FxHashMap::default()),
            rule_enabled_cache: std::cell::RefCell::new(rustc_hash::FxHashMap::default()),
            needs_ast: std::cell::Cell::new(false),
            needs_lines: std::cell::Cell::new(false),
            needs_file: std::cell::Cell::new(false),
            needs_tokens: std::cell::Cell::new(false),
            scratch_offsets: Vec::new(),
            scratch_resolved_line_info: Vec::new(),
            scratch_reports: Vec::new(),
            node_line_cache: std::cell::RefCell::new(rustc_hash::FxHashMap::default()),
        };
        // Precompute should-run values and category flags even when no file path
        // is provided so pipeline decisions (needs_ast/needs_lines) are correct.
        __reuko_precompute_should_run_map(&checker);
        checker.compute_needs_flags();
        checker
    }

    /// Mutable access to scratch offsets vector used by high-performance routines.
    #[inline]
    pub(crate) fn scratch_offsets_mut(&mut self) -> &mut Vec<(usize, usize)> {
        &mut self.scratch_offsets
    }

    /// Mutable access to scratch resolved line info vector used by high-performance routines.
    #[inline]
    pub(crate) fn scratch_resolved_line_info_mut(&mut self) -> &mut Vec<(usize, usize, usize, usize, usize)> {
        &mut self.scratch_resolved_line_info
    }

    /// Mutable access to scratch reports vector used by high-performance routines.
    #[inline]
    pub(crate) fn scratch_reports_mut(&mut self) -> &mut Vec<(usize, usize, isize)> {
    ...
    /// Run all file-level rules (functions that accept &mut Checker).
    pub fn run_file_rules(&mut self) {
        run_file_rules!(self);
    }
    ...
    pub fn with_file_path(source: &'rk [u8], config: &'rk Config, file_path: &'rk str) -> Self {
        ...
        // Precompute should-run values for all rules (generated in rule_registry.rs)
        __reuko_precompute_should_run_map(&checker);
        // Compute category flags once (e.g., whether AST phase is needed)
        checker.compute_needs_flags();
        checker
    }
    /// Build the node index by traversing the AST before running rules.
    pub fn build_index(&mut self, root: &Node<'rk>) {
        let mut visitor = CombinedVisitor {
            semantic: &mut self.semantic,
            line_index: &self.line_index,
            node_line_cache: &self.node_line_cache,
        };
        visitor.visit(root);
    }
    ...
    /// Get cached line and column for an offset, computing if not cached.
    #[inline]
    pub fn get_cached_line_column(&self, offset: usize) -> (usize, usize) {
        if let Some(&lc) = self.node_line_cache.borrow().get(&offset) {
            lc
        } else {
            let lc = self.line_index.line_column(offset);
            self.node_line_cache.borrow_mut().insert(offset, lc);
            lc
        }
    }
    ...
    fn matches_patterns(&self, patterns: &[String]) -> Option<bool> {
        ...
        // Compute a stable hash key for (file_path + patterns contents).
        ...
        if let Some(cached) = self.matches_patterns_cache.borrow().get(&key) {
            return *cached;
        }

        // Build glob matcher (fallible) and compute match result.
        ...
        let glob_set = match builder.build() {
            Ok(g) => g,
            Err(_) => {
                // Cache the failure as None so we won't repeatedly try and fail.
                // Note: None here means we couldn't perform matching.
                self.matches_patterns_cache.borrow_mut().insert(key, None);
                return None;
            }
        };
        ...
        let matched = if glob_set.is_match(path) {
            true
        } else if let Some(stripped) = file_path.strip_prefix("./") {
            glob_set.is_match(std::path::Path::new(stripped))
        } else if let Some(filename) = path.file_name() {
            glob_set.is_match(std::path::Path::new(filename))
        } else {
            false
        };

        // Cache the result for subsequent calls.
        self.matches_patterns_cache.borrow_mut().insert(key, Some(matched));
        Some(matched)
    }
    ...
    /// Check if the rule should run for the current file, using a per-rule cache entry.
    /// `config_path` is a static configuration identifier like "layout.indentation_consistency".
    #[inline]
    /// Check if a cop should run for the given BaseCopConfig, using a per-rule cache.
    /// This will prefer precompiled GlobSets if available and fall back to pattern
    /// matching when necessary.
    #[inline]
    pub fn should_run_cop_cached(&self, config_path: &'static str, base: &crate::config::BaseCopConfig) -> bool {
        if let Some(v) = self.rule_enabled_cache.borrow().get(config_path) {
            return *v;
        }
        let enabled = self.should_run_cop(base);
        self.rule_enabled_cache.borrow_mut().insert(config_path, enabled);
        enabled
    }

    /// Directly set an entry in the rule enabled cache. Used by generated precompute helper.
    pub fn set_should_run_cached(&self, config_path: &'static str, enabled: bool) {
        self.rule_enabled_cache.borrow_mut().insert(config_path, enabled);
    }

    /// Compute whether the current file needs AST/token/line phases by scanning generated
    /// arrays of config paths and the rule_enabled_cache. This is invoked after
    /// `__reuko_precompute_should_run_map` to set `needs_*` flags.
    pub fn compute_needs_flags(&self) {
        let mut needs_ast = false;
        let mut needs_line = false;
        let mut needs_token = false;
        for &cfg in crate::checker::__REUKO_AST_RULE_CONFIGS.iter() {
            if let Some(v) = self.rule_enabled_cache.borrow().get(cfg) {
                if *v {
                    needs_ast = true;
                    break;
                }
        ...
        for &cfg in crate::checker::__REUKO_LINE_RULE_CONFIGS.iter() {
            if let Some(v) = self.rule_enabled_cache.borrow().get(cfg) {
                if *v {
                    needs_line = true;
                    break;
        ...
        for &cfg in crate::checker::__REUKO_TOKEN_RULE_CONFIGS.iter() {
            if let Some(v) = self.rule_enabled_cache.borrow().get(cfg) {
                if *v {
                    needs_token = true;
                    break;
        ...
        for &cfg in crate::checker::__REUKO_FILE_RULE_CONFIGS.iter() {
            if let Some(v) = self.rule_enabled_cache.borrow().get(cfg) {
                if *v {
                    needs_file = true;
                    break;
        ...
        self.needs_ast.set(needs_ast);
        self.needs_lines.set(needs_line);
        self.needs_file.set(needs_file);
        self.needs_tokens.set(needs_token);
    }
    ...
    #[inline]
    pub fn should_run_cop(&self, base: &crate::config::BaseCopConfig) -> bool {
        let included = if let Some(gs) = &base.include_glob {
            // If we have a file path, attempt matching with several path forms.
            if let Some(fp) = self.file_path {
                let path = std::path::Path::new(fp);
                if gs.is_match(path) {
                    true
                } else if let Some(stripped) = fp.strip_prefix("./") {
                    gs.is_match(std::path::Path::new(stripped))
                } else if let Some(filename) = path.file_name() {
                    gs.is_match(std::path::Path::new(filename))
                } else {
                    // No meaningful path form; conservative default: include
                    true
                }
            } else {
                // No file path => include by default
                true
            }
        } else {
            // Fall back to pattern matching using original include patterns
            self.matches_patterns(&base.include).unwrap_or(true)
        };

        let excluded = if let Some(gs) = &base.exclude_glob {
            if let Some(fp) = self.file_path {
                let path = std::path::Path::new(fp);
                if gs.is_match(path) {
                    true
            ...
            self.matches_patterns(&base.exclude).unwrap_or(false)
        };
